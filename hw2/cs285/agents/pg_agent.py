...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        advantages = self.estimate_advantage(obs=observations, rews_list=rewards_list, q_values=q_values, terminals=terminals)
        train_log = self.actor.update(observations=observations, actions=actions, advantages=advantages, q_values=q_values)
        return train_log
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = np.cumprod(np.zeros_like(rewards) + self.gamma) / self.gamma
        list_of_discounted_returns = np.cumsum((rewards * discounts)[::-1])
        list_of_discounted_returns[:] = np.sum(list_of_discounted_returns, keepdims=True)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounts = np.cumprod(np.zeros_like(rewards) + self.gamma) / self.gamma
        rolling = np.arange(discounts.shape[0])
        discounts = np.triu(np.fliplr(np.array([np.roll(discounts[::-1], -i) for i in rolling])))
        list_of_discounted_returns = discounts @ rewards
        return list_of_discounted_returns
